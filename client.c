...
void display_device_status(UserData *user) {
    ...
    printf("%-15s | %-8s | %-12s | %s\n",
           "DEVICE NAME", "STATUS", "TARGET TIME", "REMAINING TIME");
    printf("----------------|----------|--------------|----------------\n");
    ...
    for (int i = 0; i < user->device_count; i++) {
        ...
        char notes[32];
        strcpy(notes, "Ready to Schedule");
        ...
        if (dev->target_time > 0) {
            ...
            if (diff_sec > 0) {
                ...
                status_str = "ON";
                snprintf(notes, sizeof(notes), "%ld minutes", diff_min);
            } else {
                status_str = "PASSED";
                strcpy(notes, "Schedule Cleared");
        ...
        printf("%-15s | %-8s | %-12s | %s\n",
               dev->name,
               status_str,
               (dev->target_time > 0 || strcmp(status_str, "PASSED") == 0) ? target_time_str : "N/A",
               notes);
    }
    printf("--------------------------------------------------------\n");
